...
function shallowCopy(obj) {
  // const cloneObj = { ...obj };
  // return Object.assign(cloneObj, cloneObj);
  const copy = { ...obj };
  return copy;
}
...
function makeWord(lettersObject) {
  ...
  arrLetters.forEach(([letter, indx]) => {
    indx.forEach((i) => {
      arr[i] = letter;
    });
  });
...
function sellTickets(queue) {
  const arrLast = queue[queue.length - 1];
  const arrExceptLast = queue;
  let arr = arrExceptLast.slice(0, -1);
  if (arr.length > 1) {
...
function sortCitiesArray(arr) {
  ...
  const arrCountryCity = arrCountry.sort((a, b) => {
    if (a.country === b.country) {
      return a.city.localeCompare(b.city);
    }
    return arrCountry;
  });
  return arrCountryCity;
}
